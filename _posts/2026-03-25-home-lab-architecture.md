...
기존에 활용하던 서버에 신규 서버를 더하고, 운영체제(OS) 설치부터 네트워크망 구성, 클러스터 셋업, 서비스 배포까지 모든 과정을 직접 진행했다. 재직 중인 회사의 분석 시스템과 유사한 환경을 꾸미면서 기능 개발과 운영 경험을 간접적으로 학습하고 있다. 현재 총 4대의 서버가 두 개의 클러스터로 나뉘어 운영 중이며, 그 위에서 100개 이상의 서비스가 돌아가고 있다.
