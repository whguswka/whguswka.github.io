...
현재 운영 중인 서비스 워크로드는 다음과 같이 크게 3개 계층으로 나뉜다.
...
Kubeflow[^kubeflow] 워크스페이스 엔드유저 접근성을 개선하려고 자체 개발한 프론트엔드 포털이다. 리소스 할당(CPU/RAM/GPU)과 분석 컨테이너 이미지(기본, 데이터 사이언스, 고급 등) 프로비저닝 워크플로우를 통제한다.

기본 Kubeflow 대시보드의 RBAC(Role-Based Access Control) 기능이 제한적이라 독립적인 포털을 구축했다.
...
프론트엔드 계층은 HTMX[^htmx]를 도입해 구축했다. 관리 시스템의 워크로드 특성상 복잡한 클라이언트 측 렌더링보다 명시적인 상태 관리와 경량 API 통신에 중점을 두었다.

### JupyterLab 분석 샌드박스 

프로비저닝된 사용자 개별 샌드박스 컨테이너(JupyterLab[^jupyterlab]) 내부에는 다음 도구와 구성이 미리 세팅되어 있다.
...
코드 레벨의 조작 없이 노드 기반 비주얼 에디터로 LLM 파이프라인을 설계하는 Low-code 프레임워크다. PoC(Proof of Concept) 단계의 RAG[^rag] 체인과 에이전트 구성을 빠르게 프로토타이핑한다. 인프라 클러스터 내부에 배포된 다수의 오픈소스 LLM 및 Vector DB Endpoint 설정이 미리 주입되어 있다.

### OpenWebUI

표준적인 멀티 모델 챗봇 프론트엔드다. DGX Spark 클러스터에서 할당하는 대규모 파라미터(Heavy-weight) 모델부터 Worker 노드의 소형 모델까지 vLLM API를 취합해 단일 창구에서 성능을 테스트한다.
...
현재 K3s 인프라 내부에는 100여 개 이상의 마이크로서비스가 프로비저닝되어 있다. 파편화된 컨테이너의 포트 할당 관리와 헬스체크 프로세스를 통합 모니터링하려고 개발한 커스텀 컨트롤러 뷰어다.
...
멀티 AI 에이전트 간 컴퓨팅 자원 확보 경합, 역할 충돌, 코드 오염 같은 Race Condition을 제어하려고 자체 설계한 작업 관제(Orchestration) 시스템이다. 초기 SQLite 기반에서 현재 PostgreSQL로 마이그레이션해 동시성과 안정성을 확보했다.
...
- **에이전트 신뢰도 관리**: 에이전트별 신뢰도(trust_score)를 수치화하여 작업 결과의 피드백에 따라 자동 조정하며, 위험도 높은 작업 시 교차 검증(Phase 6a)을 강제한다.
...
- **컨텍스트 브리핑**: 에이전트가 작업 시작 전 신뢰도 프로필, 실패 이력, 관련 지식을 토큰 예산 내에서 자동 수집하여 컨텍스트 단절을 최소화한다.
...
복수 클러스터 환경의 워크로드를 시각적으로 관리하려고 도입한 도구다. K3s 노드 상태, 네임스페이스별 워크로드, 리소스 사용량 등을 웹 UI에서 직관적으로 모니터링한다.
...
분산된 마이크로서비스 파드에서 생성되는 로그 스트림을 수집하고 파싱하는 Streamlit 기반 지능형 관제 환경이다. 단순 정적 로그 수집(Aggregation)을 넘어 아래의 지능적 RAG 기반 분석 시스템이 모듈화되어 있다.
...
Kubeflow Pipelines 등에 스케줄링되어 특정 트리거 조건에 따라 일괄(Batch) 수집 작업을 수행하는 워커 프로세스 목록이다.
...
원천 데이터가 1차 스토리지인 PostgreSQL 인스턴스에 적재되면, Data Sync ETL 워크플로우가 트리거되어 이를 분석계 DW(Teradata)로 적재해 예측 파이프라인의 연산 가용성을 확보한다.

### 가격 시계열 예측 ML 파이프라인

ML옵스(MLOps) 아키텍처를 비교 검증하고 인프라 프로파일링을 확보하려고 설계한 시계열 예측 모형 파이프라인이다.
...
이 파이프라인은 직접적인 트레이딩 수익 창출보다 데이터베이스 연산(In-Database) 아키텍처와 분리된 애플리케이션 계층 연산 간의 성능 프로파일링 지표 추출을 주 목적으로 한다. 네트워크 구간의 Data I/O 병목과 GPU 스케줄링 메모리 오버헤드 측정 등 실측 아키텍처 검증 환경으로 워크로드를 할당한다.

### Docling Extractor API

PDF, 바이너리 문서 포맷 등을 입력받아 OCR(광학 문자 인식) 파이프라인과 레이아웃 파싱 알고리즘으로 구조화된 마크다운/JSON 포맷을 반환하는 마이크로 API 컨트롤러다. 지식 베이스 구축 인제스천(Ingestion) 단계의 전처리기를 담당한다.

### Teradata Vector 검색 PoC

Teradata Native Vector 쿼리 활용성을 검증하는 PoC(Proof of Concept) 서비스 애플리케이션이다. VLM/LLM 기반의 텍스트 임베딩 모델을 연동하며, 유클리디안 거리와 코사인 유사도 검색 연산을 애플리케이션 단에서 오케스트레이션하는 UI 인터페이스로 운용한다.
...
DGX Spark 2노드 클러스터 상태를 모니터링하는 전용 대시보드다. vLLM 서비스 상태, 메모리 사용량, 모델 로딩 상태 등을 활성화된 프로브로 수집해 시각화한다.
...
앞으로 시스템 아키텍처 수준에서 진행할 추가 연동과 고도화 파이프라인 마일스톤은 다음과 같다.
...
이후 연재 포스트에서는 개별 마이크로서비스와 파이프라인 배포 때 겪은 장애 트러블슈팅 내역, 그리고 아키텍처적 Trade-off 의사 결정 과정을 세부적으로 다룬다.
