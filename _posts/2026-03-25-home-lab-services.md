...
**접속**: `http://192.168.0.156:30088`

Stable Diffusion 기반 이미지 생성 노드 에디터. 87 서버의 GPU에서 실행된다.
...
graph LR
    subgraph UserServices["사용자용 서비스"]
        AP[Analysis Portal<br/>:31080]
        JL[JupyterLab<br/>Notebooks]
        LF[Langflow<br/>:31180]
        OW[OpenWebUI<br/>:8080]
        SX[SearXNG<br/>:30889]
    end

    subgraph AdminServices["관리용 서비스"]
        SP[Service Portal<br/>:31082]
        ATH[Agent Task Hub<br/>:30880]
    end

    subgraph DataML["데이터/ML"]
        KFP[Kubeflow Pipelines<br/>:31081]
        COL[Data Collectors]
        STK[Stock Prediction<br/>Dashboard :31089]
    end
