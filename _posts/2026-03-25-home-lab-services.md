...
현재 K3s 인프라 내부에는 100여 개 이상의 마이크로서비스가 프로비저닝되어 있다. 파편화된 컨테이너의 포트 할당 관리 및 헬스체크 프로세스를 통합 모니터링하기 위해 개발한 커스텀 컨트롤러 뷰어다.
...
K3s 클러스터의 선언적 상태 관리를 위해 도입한 GitOps 도구다. Git 리포지토리의 매니페스트를 Single Source of Truth로 설정하여, 클러스터 상태와의 Drift를 자동 감지하고 동기화한다.

### Rancher (클러스터 관리 UI)

복수 클러스터 환경의 워크로드를 시각적으로 관리하기 위해 도입한 도구다. K3s 노드 상태, 네임스페이스별 워크로드, 리소스 사용량 등을 웹 UI에서 직관적으로 모니터링한다.

### Ops Intel (운영 정보 수집기)

에이전트 작업 시 필요한 운영 정보(K8s 리소스, 파드 상태, 로그 등)를 자동 수집하여 ATH 지식베이스에 적재하는 데이터 수집 에이전트다.
