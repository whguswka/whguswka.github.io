...
122B FP8의 안정적인 운영 경험을 바탕으로, 더 큰 모델로의 확장을 시도했다. `Intel/Qwen3.5-397B-A32B-Instruct-int4-AutoRound`로 전환하여 현재까지 안정적으로 운영 중이다.

### 전환 이유

Qwen3.5의 397B 버전은 122B 대비 활성 파라미터가 32B로 증가하여(활성 10B → 32B), 복잡한 추론과 긴 컨텍스트 처리에서 유의미한 품질 향상이 기대되었다. Intel의 AutoRound int4 양자화는 기존 AWQ/GPTQ 대비 정밀도 손실이 적어 DGX Spark 환경에서도 안정적으로 동작했다.
...
최신 DeepSeek V4 Flash 모델의 성능 지표가 우수하여 로컬 서비스 전환을 시도했으나, **GB10 GPU의 SM(Streaming Multiprocessor) 12.1 호환성 문제**로 인해 vLLM 로딩에 실패했다.

### 실패 원인

DGX Spark의 GB10 칩은 Blackwell 아키텍처 기반이지만 SM 버전이 12.1로, DeepSeek V4 Flash가 요구하는 CUDA 연산 기능과 호환되지 않았다. 구체적으로 vLLM의 모델 로드 단계에서 커널 레벨 오류가 발생했다.
...
8번의 모델 전환을 거치면서 얻은 가장 큰 교훈은 세 가지다:
...
현재 Qwen3.5-397B int4-AutoRound + vLLM 조합은 262K 컨텍스트, tool calling, reasoning을 모두 안정적으로 지원하며, 6개의 AI 에이전트가 동시에 사용하는 환경에서 실용적으로 동작하고 있다.
