...
모델이 안정적으로 동작하는지 확인하고 컨텍스트 길이를 확장했다. 코딩 에이전트로 쓰려면 큰 파일이나 여러 파일의 맥락을 한 번에 넘겨야 해서 컨텍스트가 길수록 유리하다.
...
vLLM은 GGUF 포맷을 지원하지 않는다. safetensors 기반의 양자화 포맷(AWQ, GPTQ, FP8 등)만 지원한다. GGUF를 서빙하려면 llama.cpp의 `llama-server`를 사용해야 한다.
...
397B는 235B보다 모델 규모가 크고, MXFP4 양자화도 AWQ 4-bit보다 정밀도가 높아 가중치 크기가 더 컸다. 그래서 생성 속도가 체감될 만큼 느렸다.

더 큰 문제는 llama.cpp 환경에서 tool calling을 설정하는 과정이 vLLM만큼 간단하지 않다는 점이다. AI 코딩 에이전트의 핵심 기능인 함수 호출이 안정적으로 동작하지 않으면 실용성이 크게 떨어진다.

### 롤백 결정

속도와 tool calling 안정성을 고려해 vLLM으로 롤백하기로 했다. llama.cpp 바이너리와 Qwen3.5-397B 모델 파일(202GB)을 완전히 삭제하고, 기존 Qwen3-235B AWQ로 복원했다.
...
FP8은 AWQ 4-bit보다 정밀도가 높다(8-bit vs 4-bit). 같은 모델이라면 FP8이 메모리를 더 많이 사용하지만, 122B는 235B보다 원본 크기가 작아 FP8로도 DGX Spark에 안정적으로 올라갔다. 컨텍스트를 262K까지 늘릴 만큼 여유가 있었다.
...
SWE-Bench 점수와 달리, 실제 일상 사용에서는 심각한 문제가 드러났다.
...
이런 문제들은 코딩 에이전트의 사용 편의성을 크게 떨어뜨렸다. 벤치마크에서 측정하는 "코드 패치를 올바르게 생성하는 능력"과 "사용자와 자연스럽게 협업하는 능력"은 서로 다른 차원의 문제라는 점을 체감했다.

---

## 6단계: Qwen3.5-122B FP8 복귀

MiniMax-M2.5의 실사용 문제 때문에, 4단계에서 안정적으로 운영한 Qwen3.5-122B FP8으로 롤백하기로 했다.

### 롤백 결정

MiniMax-M2.5를 약 2주간 사용했지만 문맥 이해, 코드 품질, 언어 일관성 세 영역 모두 개선되지 않았다. 프롬프트 엔지니어링으로 일부 완화했을 뿐, 근본적인 해결책은 아니었다.
...
**교훈**: 벤치마크 점수는 모델 선택의 필요조건일 뿐 충분조건은 아니다. 특히 코딩 에이전트 용도로는 SWE-Bench 같은 자동화된 평가보다, 지시 이행 정확도·코드 오류율·언어 일관성 같은 실사용 지표가 더 중요하다.
...
vLLM은 OpenAI 호환 API를 제공해서, 기존 OpenAI API 지원 도구의 엔드포인트만 바꾸면 바로 쓸 수 있다.
...
GPU 메모리 부족(OOM)이 발생하면 vLLM 프로세스가 갑자기 종료된다. UMA 환경에서는 `nvidia-smi`로 메모리 사용량을 모니터링하기 어려워 예측이 힘들다.
