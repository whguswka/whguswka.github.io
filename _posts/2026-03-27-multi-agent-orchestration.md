...
개인 인프라인 홈랩[^homelab]에서 운영하는 서비스가 100개를 넘으면서, 한 번에 여러 작업을 동시에 진행해야 하는 상황이 잦아졌다. 서비스 포털의 버그를 수정하면서 동시에 단위 데이터 파이프라인을 이전하고, 그 사이에 DGX Spark[^dgx-spark]의 vLLM[^vllm] 설정도 같이 변경해야 하는 식이다.
...
과거에는 WSL 환경의 별도 에이전트 등 다양한 체계도 실험했으나, 역할과 모델 특성을 기준으로 에이전트를 분화하는 현재의 6노드 구성으로 정착했다. 특히 자체 호스팅 LLM(Hermes)의 도입과 Oh-My-OpenAgent를 통한 다중 모델 라우팅(OpenCode/omo)이 주요 전환점이었다.
...
에이전트 수가 늘어나면서, 단순히 작업을 분배하는 것을 넘어 **동일한 질문을 복수 에이전트에 동시 전달하고 결과를 합성하는** 다관점 검증 패턴이 필요해졌다. 이를 위해 Claude Code(3호기)에 `hocg`(Hermes + omo + OpenClaw + Claude 합성) 스킬을 개발했다.
...
Claude Code(3호기)가 사용자 요청을 분석하여 세 워커에게 동시에 프롬프트를 전달한다. 각 워커는 강점에 따라 역할이 분화된다:

- **Hermes**: 깊은 추론, 긴 컨텍스트 분석, 아키텍처 리스크 평가
- **OpenCode (omo)**: 코딩, 도구 호출, 단일 파일 변경
- **OpenClaw**: 일반 추론, UX 관점, 문서 표현

세 응답이 모두 도착하면 Claude Code가 합성을 수행하여 공통 결론, 동의/불일치 영역, 추천안을 도출한다. Claude Code는 주로 합성 역할만 담당하므로 API 쿼터 소모를 최소화할 수 있다.

### 안전 장치

외부 워커 호출 시 무한 대기를 방지하기 위해 다층 타임아웃 구조를 적용했다:
