...
최종 결과물은 `feature/*` 브랜치에서 MR을 생성해 main에 merge하면, 변경된 서비스 이미지를 자동으로 빌드해 Harbor에 push하고 K3s에 배포까지 완료하는 파이프라인이다.
...
GitLab CE를 K3s 위에 단일 Pod(Omnibus)로 배포했다. Helm Chart 대신 직접 manifest를 작성한 이유는 GitLab Helm Chart가 수십 개의 마이크로서비스로 분리되어 리소스 소비가 너무 많기 때문이다. Omnibus는 모든 컴포넌트가 하나의 컨테이너에 포함되어 관리가 단순하다.
...
Longhorn PVC 3개로 데이터를 분리해 관리한다.
...
모든 내부 서비스가 동일한 LDAP 계정을 사용한다. GitLab도 마찬가지다.
...
LDAP 사용자가 GitLab에 처음 로그인하면 계정이 자동 생성된다. SSH 키 등록과 프로젝트 접근 권한 등은 GitLab UI에서 관리한다.

### 트러블슈팅: OOMKill과 keywatcher EOF

배포 초기, GitLab 백업을 실행하면 메모리 피크로 Pod이 OOMKill(exit code 137)되는 문제가 반복됐다. 동시에 `gitlab-workhorse` 로그에는 `keywatcher: pubsub receive: EOF`가 계속 출력됐다.

원인은 두 가지다.
- Puma 워커 프로세스와 Sidekiq 동시성이 기본값으로 과도하게 설정됐다.
- Redis의 `timeout 60` 설정으로 pubsub 유휴 연결이 주기적으로 끊겼다.
...
리소스 상한을 `16Gi → 20Gi`로 올렸다. 이후 백업이 정상적으로 완료되었고, keywatcher EOF도 재발하지 않았다.
...
Kubernetes Executor를 사용한다. CI/CD 잡이 실행될 때마다 Runner가 K3s 클러스터에 임시 Pod을 생성해 빌드를 실행하고 Pod을 삭제한다. 덕분에 전용 빌드 서버를 운영할 필요가 없다.
...
모노레포의 핵심은 **변경된 서비스만 빌드**하는 일이다. `changes` 키워드를 활용한다:
...
services/agent-hub-server/` 디렉토리 내 파일이 변경될 때만 해당 서비스의 빌드 잡이 트리거된다. 10개 서비스 중 변경된 1개만 빌드하므로 파이프라인 시간이 단축된다. 이미지 빌드는 `services/` 아래 Dockerfile 기준으로 진행하며, 배포는 `k8s/` 아래 매니페스트나 기존 Deployment의 image tag를 변경해 처리한다.

다만 공용 모듈, 루트 레벨 설정, 공통 베이스 이미지 변경은 `changes` 규칙만으로 감지하기 어렵다. 실제 운영에서는 서비스별 규칙과 공용 경로 규칙을 함께 관리해야 한다.
...
Kaniko는 Docker 데몬 없이 Dockerfile을 빌드하는 Google 오픈소스 도구다. K3s 환경에서 DinD(Docker-in-Docker)를 피하려고 선택했다.
...
빌드가 완료되면 kubectl로 K3s 클러스터에 배포한다.
...
kubectl set image`로 이미지 태그를 변경하면 Kubernetes가 롤링 업데이트를 수행한다. `rollout status`로 배포 완료를 대기하며, 120초 내에 완료되지 않으면 실패로 처리한다.

kubectl이 K3s 클러스터에 접근하려면 kubeconfig가 필요하며, 이 역시 CI Variable로 관리한다.

현재는 단순성과 속도를 위해 `kubectl set image` 기반으로 운영하지만, 선언형 GitOps(ArgoCD)와 비교하면 drift 관리가 어렵다. 장기적으로는 ArgoCD와의 역할 분리를 더 명확히 정리해야 한다.

### GitHub 미러링

내부 GitLab은 외부 접근이 불가능하므로, 공개할 프로젝트는 GitHub으로 미러링한다:
...
GitHub 미러는 GitLab 브랜치를 기준으로 상태를 강제 동기화하므로, mirror 대상 브랜치에만 `--force`를 사용한다. GitLab에서 rebase나 force push가 발생하면 GitHub 히스토리와 달라져 일반 push로는 실패하기 때문이다.
...
GitLab CI/CD에서 YAML 설정 중 마주친 예상치 못한 문제들:
...
이 에러는 GitLab의 YAML lint에서도 잡히지 않았고, 파이프라인을 실행해야 발견되어 디버깅에 시간이 걸렸다.
...
전체 파이프라인 소요 시간은 서비스마다 다르지만, 보통 빌드 2~4분과 배포 30초 정도 걸린다.
...
모든 MR에서 gitleaks가 실행되어 하드코딩된 API 키, 비밀번호, 토큰 등을 탐지한다. 실제로 몇 번의 commit 전 `.env` 파일이 포함된 사례를 잡아냈다.
...
Protected` 설정으로 main 브랜치에서만 쓰게 하고, `Masked`로 로그에 노출되지 않게 한다.

---

## 마무리

온프레미스에서 CI/CD를 구축하면 클라우드 서비스(GitHub Actions, GitLab.com)와 달리 인프라 전체를 직접 관리해야 한다. GitLab 서버 메모리, Runner 리소스, Harbor 스토리지, TLS 인증서까지 모두 챙겨야 하기 때문이다. 하지만 그만큼 전체 플로우를 완전히 이해하고 제어할 수 있다.
...
앞으로 더 많은 서비스를 모노레포에 편입시켜 CI/CD 커버리지를 넓힌다.

다음 글에서는 이 인프라의 또 다른 축인 [MCP(Model Context Protocol) 서버](/development/mcp-practical-guide/)를 다룬다. AI 코딩 에이전트가 ATH와 Service Portal에 직접 접근하는 인터페이스를 어떻게 설계하고 구현했는지 정리한다.