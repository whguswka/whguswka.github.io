...
하지만 에이전트가 CLI를 호출하는 과정에서 구조적인 한계가 드러났다. 에이전트가 `bash` 도구로 `ath task create --title "..."` 명령을 실행하면, 셸 호출 → 스크립트 실행 → HTTP 요청 → 응답 파싱 경로를 거친다. 이 과정 어디에서든 문제가 생길 수 있으며, 에이전트는 셸의 exit code와 stdout 문자열을 직접 해석해야 한다. 제목에 특수문자가 포함되면 셸 이스케이핑이 깨지거나, 응답 JSON을 제대로 파싱하지 못하는 에이전트도 있었다.

차이는 **도구 발견(discovery)** 문제다. CLI 방식에서는 에이전트의 규칙 파일(GEMINI.md, CLAUDE.md 등)에 "이 CLI를 이렇게 호출하라"고 일일이 적어야 한다. 규칙 파일이 바뀌면 모든 에이전트의 규칙을 동시에 갱신해야 하며, 에이전트가 규칙을 참조하지 않으면 도구의 존재 자체를 알 수 없다.

이 문제를 해결한 것이 **MCP(Model Context Protocol)**다. MCP 서버를 구축하면 에이전트가 ATH 기능을 네이티브 도구(tool)로 직접 쓴다. `ath_task_create`가 `grep`이나 `file_read`와 같은 레벨의 도구가 된다. 서버가 도구 목록과 파라미터 스키마를 자동으로 노출해 에이전트는 별도 규칙 없이도 사용 가능한 도구를 인식한다.

ATH는 CLI와 MCP를 이중 인터페이스로 제공한다.
...
이 글은 MCP의 핵심 개념을 짧게 정리하고, 실제로 두 개의 MCP 서버를 구현한 과정과 운영 경험을 다룬다.
...
MCP(Model Context Protocol)는 Anthropic이 2024년 11월에 제안했고, 이후 여러 에이전트·도구 생태계가 채택하고 있는 프로토콜이다. AI 에이전트가 외부 시스템을 표준화된 방식으로 호출하도록 돕는 인터페이스다.
...
MCP 이전에는 에이전트가 외부 API를 호출하려면 다음과 같은 방법이 필요했다.
1. bash로 curl 명령을 실행하거나
2. Python 스크립트를 작성해서 실행하거나
3. 에이전트 규칙 파일에 API 호출 방법을 길게 설명하는 방식이다.

해야 했다. MCP는 이 과정을 **에이전트의 네이티브 도구**로 구현한다.
...
이 글에서 구현한 두 MCP 서버는 **Tool**만 사용한다. 에이전트가 작업을 등록하고 지식을 검색하며 서비스를 조회하는 과정은 모두 Tool 호출로 이루어진다.
...
같은 MCP 서버에서 두 방식을 동시에 제공해야 한다. Claude Code는 SSE만 지원하며, Antigravity와 OpenCode는 StreamableHTTP를 사용하기 때문이다.

두 방식을 별도 서버로 분리하는 방법도 있었으나, 그렇게 하면 서버가 2배로 늘어 포트 관리, 배포 파이프라인, 헬스체크를 모두 이중으로 해야 한다. ATH MCP와 Service Portal MCP를 합하면 4개 서버가 되는 셈이다. 단일 서버에서 경로(`/sse`, `/mcp`)로 분기하면 배포 단위는 하나로 유지하면서 두 클라이언트를 모두 지원할 수 있다. 운영 복잡도를 줄이려고 듀얼 트랜스포트를 선택했다.
...
ATH(Agent Task Hub)에는 이미 REST API가 있다. MCP 서버는 이 API를 감싸는(wrapping) 얇은 레이어다. 핵심 역할은 다음과 같다.
...
도구는 `server.tool()` 메서드로 등록한다. Zod 스키마로 파라미터를 정의하면 MCP SDK가 자동으로 JSON Schema를 생성하며, 에이전트는 이를 바탕으로 올바른 파라미터를 호출한다:
...
하나의 HTTP 서버에서 SSE와 StreamableHTTP를 동시에 제공하는 작업이 이 MCP 서버에서 가장 까다로웠다.
...
핵심은 `createMcpServer()`를 팩토리 패턴으로 분리한 점이다. SSE는 연결마다 새로운 MCP 서버 인스턴스가 필요하고, StreamableHTTP는 세션 단위로 재사용해야 한다. 단일 MCP 서버 인스턴스를 공유하면 세션 간 상태가 충돌한다.

### 세션 관리와 메모리 누수 방지

SSE는 HTTP 연결이 유지되는 동안 세션이 지속된다. 하지만 에이전트가 비정상적으로 종료되어도 연결이 그대로 남는 경우가 발생한다. 그래서 TTL 기반의 정리 기능을 구현했다:
...
이 정리 로직이 없으면 장시간 운영할 때 세션이 계속 쌓여 메모리가 늘어난다.
...
Service Portal은 28개 이상의 내부 서비스를 관리하는 대시보드다. AI 에이전트가 새 서비스를 배포할 때 Service Portal에 등록해야 하는데, 이 과정을 에이전트가 직접 처리하도록 MCP 서버를 만들었다.
...
ATH MCP의 9개 도구보다 단순하다. Service Portal MCP는 레지스트리 CRUD에만 집중한다.

### 인증 처리

Service Portal API는 인증이 필요하다. MCP 서버 내부에서는 `X-Internal-Key` 헤더로 인증을 처리한다.
...
에이전트는 인증을 신경 쓰지 않고 도구만 호출하면 된다. 인증 키는 MCP 서버의 환경변수로 관리되어 에이전트에게 노출되지 않는다.

### 구현 특이점: JavaScript

ATH MCP는 TypeScript인데 Service Portal MCP는 순수 JavaScript로 작성했다. 초기 구현 속도를 위해 JavaScript를 선택했고, 현재 운영에 무리가 없어 그대로 유지하고 있다.

두 MCP 서버의 구조는 거의 비슷하다.
...
## 최근 추가한 MCP 서버들 (2026-07 업데이트)

이 글을 처음 쓸 때는 MCP 서버가 ATH와 Service Portal 두 개였다. 이후 같은 패턴 — 기존 REST API를 얇게 감싸는 MCP 레이어 — 을 그대로 재사용해 서버를 몇 개 더 추가했다. 클러스터가 커지면서(현재 내부 서비스 100개 이상) 에이전트가 도구로 직접 다뤄야 할 내부 시스템도 함께 늘어난 결과다. 방법론 자체는 앞 절과 동일하므로, 여기서는 각 서버가 어떤 문제를 풀었는지만 기능 중심으로 짧게 정리한다.

### Monitoring MCP

문제 정의부터 짚는다. 에이전트에게 장애 대응이나 상태 점검을 맡기면, 정작 관제 데이터(파드 상태, 메트릭, 최근 이벤트)는 사람이 대시보드에서 읽어 요약해줘야 했다. 에이전트는 눈이 없어서, 사람이 중간에서 관제 화면을 대신 봐주는 병목이 생긴 것이다.

Monitoring MCP는 이 관제 데이터를 MCP 도구로 노출한다. 에이전트가 대시보드를 사람처럼 훑는 대신, "지금 이상 징후가 있는 서비스" 같은 질의를 도구 호출로 던지고 구조화된 응답을 받는다. ATH MCP와 마찬가지로 MCP 서버는 관제 백엔드를 감싸는 얇은 변환 레이어일 뿐이고, 판단 로직은 넣지 않았다. 관제 데이터의 해석은 에이전트의 몫으로 남긴다.

### OpenClaw RAG MCP

내부 지식 베이스를 벡터 검색으로 조회하는 RAG 파이프라인을 MCP 도구로 감쌌다. 에이전트가 답을 만들기 전에 관련 문서를 스스로 검색(retrieval)하도록 하는 것이 목적이다. ATH MCP의 `ath_knowledge_search`가 ATH 지식만 대상으로 하는 반면, 이 도구는 더 넓은 문서 코퍼스를 벡터 유사도로 검색한다는 점이 다르다. 검색 대상과 랭킹 로직은 RAG 백엔드에 두고, MCP 서버는 질의를 전달하고 결과를 포맷팅하는 역할만 맡는다.

### 소비 에이전트 구성의 변화

MCP 도구를 쓰는 에이전트 구성도 이 글을 쓴 뒤로 바뀌었다. GPT-5.x 계열 워커가 새로 합류했고, 반대로 백엔드 추론 품질이 떨어진 워커 하나는 [의견 수렴 파이프라인](/development/multi-agent-orchestration/)에서 제외했다. 어느 경우든 MCP 서버는 손대지 않았다. 트랜스포트와 도구 스키마를 그대로 둔 채 클라이언트 목록만 조정하면 되었기 때문이다. 애초에 CLI 대신 MCP를 택한 이유가 이런 확장성이었다.

---

## 에이전트별 MCP 연결 설정

각 에이전트는 설정 파일에 MCP 서버 연결을 정의한다.

### Antigravity (Gemini)

mcp_config.json` 파일에 URL 기반으로 설정한다.
...
MCP 서버에 health check 엔드포인트를 구현했다:
...
OpenClaw이 CLI를 사용하는 이유는 워크플로우 기반 실행 구조에서 셸 명령이 기본 인터페이스이기 때문이다. MCP 클라이언트를 내장하지 않은 환경에서는 CLI가 유일한 선택지다.
...
**MCP 해결**: 도구 호출 시 제목이 JSON 문자열로 전달되어 셸 이스케이핑 문제가 발생하지 않는다.

**문제**: OpenCode(로컬 LLM 기반)가 CLI 도구를 호출할 때 응답 JSON을 파싱하지 못해 다음 행동을 결정하지 못하는 사례가 있었다.

**MCP 해결**: MCP 응답은 SDK가 구조화해 반환하므로, 에이전트가 별도로 파싱하지 않고 결과를 사용한다.

---

## 나만의 MCP 서버를 만들기 위한 가이드

기존 REST API가 있는 시스템에 MCP 서버를 추가하려면 다음 패턴을 따르면 된다.
...
SSE와 StreamableHTTP를 동시에 제공하면 대부분의 에이전트 클라이언트를 지원한다. `/sse`와 `/mcp` 두 개의 엔드포인트를 만든다.

### 4. 도구 설명에 투자하기

에이전트가 도구를 올바르게 사용할지는 도구 설명에 달려 있다. 몇 가지 팁을 소개한다.
...
Claude Code가 장시간 작업하면 SSE 연결이 끊길 때가 있다. 네트워크 타임아웃이나 프로세스 중단 등 원인은 다양하다. 세션 TTL 기반 정리를 구현하고, health check로 현재 활성 세션 수를 모니터링하자.
...
처음에는 ATH의 모든 API를 도구로 노출하려 했다. 하지만 도구가 많아지면 에이전트의 도구 선택 정확도가 떨어진다. 핵심 기능만 노출하고, 고급 기능은 필요할 때 추가하는 편이 낫다.
...
MCP는 AI 에이전트와 외부 시스템을 연결하는 효율적인 방법이다. REST API를 직접 호출하는 것보다 안정적이며, 에이전트의 네이티브 도구로 취급되어 사용자 경험도 좋다.

현재 여러 MCP 서버를 운영하며, 4개의 AI 에이전트가 매일 수십 번씩 이 도구들을 호출하고 있다. CLI 방식보다 에러율이 크게 줄었고, 에이전트가 ATH 규칙을 더 잘 따른다.

기존 REST API가 이미 정리되어 있다면 MCP 서버를 추가하는 일은 비교적 짧은 시간 안에 가능하다. SDK가 대부분의 복잡한 과정을 처리하기 때문이다. AI 에이전트를 실무에 활용하는 환경이라면 MCP 도입을 검토할 만하다.