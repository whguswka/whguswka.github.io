...
이러한 개선의 결과로 ATH 규약 전체 준수율은 초기 ~65%에서 ~74%로 상승했다. 여전히 파일 Lock 등에서 개선의 여지가 있으나, 작업 충돌 빈도와 중복 실행 비율은 확연히 줄어드는 추세다. 현재 6개 에이전트(Antigravity, Claude Code, OpenClaw, Agent PoC, OpenCode/omo, Hermes)가 동시 운영되는 환경에서 ATH는 핵심 오케스트레이션 인프라로 기능하고 있다.
